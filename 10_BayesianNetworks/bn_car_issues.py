    ...
    def calculate_marginal_probability(self):
        ...
        for assignment in self.assignments:
            for k, v in self.probability_table.items():
                self.marginal_probabilities[self.get_assignment_index(assignment)] += v[
                    self.get_assignment_index(assignment)]
...
class BayesianNetwork(object):
    ...
    def get_joint_probability(self, values):
        ...
        for k, v in values.items():
            variables.append(self.varsMap[k])
        ...
        for k, v in values.items():
            ...
            for parent in variables:
                if var.is_child_of(parent):
                    parents.append(parent)
            if len(parents) > 0:
                for parent in parents:
                    self.joint_probability.append(var.get_conditional_probability(v, values[parent.name]))
            else:
                self.joint_probability.append(var.get_marginal_probability(v))
        print(self.joint_probability)
        return math.prod(self.joint_probability)
...
def car_issue():
    ...
    EM_t = {(): (0.8, 0.2)}
    FTL_t = {(): (0.7, 0.3)}
    V_t = {('false',): (0.9, 0.1), ('true',): (0.3, 0.7)}
    SMS_t = {
        ('false', 'false'): (0.3, 0.7),
        ('true', 'false'): (0.4, 0.6),
        ('false', 'true'): (0.7, 0.3),
    ...
    DT = Variable('DamagedTire', ('false', 'true'), DT_t)
    EM = Variable('ElectronicsMalfunctioning', ('false', 'true'), EM_t)
    FTL = Variable('FuelTankLeaking', ('false', 'true'), FTL_t)
    V = Variable('Vibrations', ('false', 'true'), V_t, [DT])
    SMS = Variable('SlowMaxSpeed', ('false', 'true'), SMS_t, [DT, EM])
    HC = Variable('HighConsumption', ('false', 'true'), HC_t, [DT, EM, FTL])
    ...
    print('Probability of it being DT')
    ...
    print('Probability of it being EM')

    conditionals_vars = {'ElectronicsMalfunctioning': 'true'}
    conditionals_evidents = {'Vibrations': 'true', 'SlowMaxSpeed': "true", 'HighConsumption': "false"}

    print_conditional_probability(network, conditionals_vars, conditionals_evidents)
    print('Probability of it being FTL')
